...
def finetune_after_prune(net, epochs, batch_size, lr, reg, log_every_n=50):
    print('==> Preparing data..')
    transform_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
    ...
    optimizer = optim.SGD(net.parameters(), lr=lr, momentum=0.875, weight_decay=reg, nesterov=False)
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[int(epochs*0.25), int(epochs*0.75)], gamma=0.1)
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for n, m in net.named_modules():
                if isinstance(m, PrunedConv):
                    m.conv.weight.data = m.conv.weight.data * m.mask
                elif isinstance(m, PruneLinear):
                    m.linear.weight.data = m.linear.weight.data * m.mask
            train_loss += loss.item()
            _, predicted = outputs.max(1)
            total += targets.size(0)
            correct += predicted.eq(targets).sum().item()
...
def test(net):
    ...
    return val_acc # Slight ammendment
