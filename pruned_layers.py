...
class PruneLinear(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        thresh = np.percentile(np.abs(flattened_weights), q)
        # Generate a mask to determine which weights should be pruned (Your code: <=3 Lines)
        self.mask = np.abs(flattened_weights) >= thresh if q == 0 else np.abs(flattened_weights) > thresh
        # Multiply weight by mask (Your code: 1 Line) 
        np_weight = flattened_weights*self.mask
        # Copy back to linear.weight and assign to device (Your code: 1 Line)
        self.linear.weight.data = torch.reshape(torch.from_numpy(np_weight), self.linear.weight.data.shape).to(device)
        # Compute sparsity (Your code: 1 Line)
        self.sparsity = np.mean(self.mask)
        # Copy mask to device for faster computation [Your code: 1 Line]
        self.mask = torch.reshape(torch.from_numpy(self.mask), self.linear.weight.data.shape).to(device)
    ...
    def prune_by_std(self, s=0.25):
        ...
        # Generate a mask to determine which weights should be pruned (Your code: <=3 Lines)
        self.mask = np.abs(flattened_weights) >= thresh
        # Multiply weight by mask (Your code: 1 Line)
        np_weight = flattened_weights*self.mask
        # Copy back to linear.weight and assign to device (Your code: 1 Line)
        self.linear.weight.data = torch.reshape(torch.from_numpy(np_weight), self.linear.weight.data.shape).to(device)
...
class PrunedConv(nn.Module):
    ...
    def prune_by_percentage(self, q=5.0):
        ...
        self.conv.weight.data = torch.reshape(torch.from_numpy(np_weight), self.conv.weight.data.shape).to(device)
        # Compute sparsity (Your code: 1 Line) 
        self.sparsity = np.mean(self.mask)
        # Copy mask to device for faster computation (Your code: 1 Line) 
        self.mask = torch.reshape(torch.from_numpy(self.mask), self.conv.weight.data.shape).to(device)
